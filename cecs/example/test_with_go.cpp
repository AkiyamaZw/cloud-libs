...
RenderComponent::RenderComponent(GameObject *_go) : go(_go) {}
...
AnimatorComponent::AnimatorComponent(GameObject *obj) : go(obj) {}
...
void test_main() {
  ...
  auto show_result = [](const std::string &name,
    ...
    std::cout << std::format("tag: [{:>12}], avg: {:.1f} us, total: {} us",
                             name, data.first, data.second)
              << std::endl;
  };

  auto calc_average = [average_cnt, entity_cnt,
                       epochs](TestFunc func) -> std::pair<float, size_t> {
