...
public:
  ...
  // ↓ ↓ ↓ entity ↓ ↓ ↓
  template <typename... C> EntityID create_entity();

  template <typename... C> std::vector<EntityID> create_entities(size_t size);
  ...
  //  ↓ ↓ ↓ component ↓ ↓ ↓
  template <typename C> C &get_component(const EntityID &id);

  template <typename C> bool has_component(const EntityID &id) const;

  template <typename C> C *get_singleton_component();

  template <typename C> C *set_singleton_component();

  template <typename C> C *set_singleton_component(C &&singleton);
  // ↑ ↑ ↑ component ↑ ↑ ↑

protected:
  // ↓ ↓ ↓ archetype ↓ ↓ ↓

  template <typename... C> internal::Archetype *get_or_create_archetype();

  template <typename... C> internal::Archetype *create_archetype();
