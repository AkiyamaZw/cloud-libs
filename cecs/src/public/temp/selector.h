...
struct Selector {
  ...
  template <typename... C> Selector &with() {
  ...
  void unpack_chunk(type_list<Args...> type, internal::Chunk *chunk,
                    Func &&func) {
    // #ifdef DEBUG
    ...
    for (size_t i = 0; i < chunk->get_allocated_size(); ++i) {
      if (chunk->get_alive(i)) {
        func(std::get<decltype(chunk->get_chunk_component<Args>())>(tup)[i]...);
      }
    }
  }

  template <typename Func> Selector &for_each(Func &&func) {
  ...
  template <typename... C> static View query(Registry &registry) {
  ...
  static View create_view(Registry &registry, const MetaTypeList &metas,
                          MaskType mask);
};
