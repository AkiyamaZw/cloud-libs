	...
	: queries_per_frame_(queries_per_frame)
{
	...
	timestamps_data_ = reinterpret_cast<uint64_t *>(
		raw_memory + sizeof(GPUTimestamp) * queries_per_frame * max_frames);
	Reset();
}
...
bool GPUTimestampManager::HasValidQueries() const { return current_query_ > 0 && depth_ == 0; }
...
{
	memcpy(timestamps_to_fill,
		   &timestamps_[current_frame * queries_per_frame_],
		   sizeof(GPUTimestamp) * current_query_);
