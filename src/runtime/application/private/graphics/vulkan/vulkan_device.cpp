...
typedef struct _GpuDevice
{
    ...
    cloud::ResourcePool descriptor_set_layout{descriptor_layout_pool_size, sizeof(DescriptorSetLayout)};
...
void CreateDebugExt()
{
    ...
    const auto &result = std::find_if(extensions.begin(),
                                      extensions.end(),
                                      [](const auto &rhs) {
                                          return !strcmp(rhs.extensionName,
                                                         VK_EXT_DEBUG_UTILS_EXTENSION_NAME);
                                      });
...
void CreatePhysicalDevice()
{
    ...
    VkResult succ = vkEnumeratePhysicalDevices(g_vulkan_device.instance,
                                               &num_physical_device,
                                               nullptr);
    check_vk(succ);
    ...
    for (uint32_t index = 0; index < num_physical_device; ++index)
    {
        ...
        const VkPhysicalDeviceType device_type = g_vulkan_device.physical_device_properties.
                                                                 deviceType;
        ...
        {
            if (get_family_queue(gpus[index],
                                 g_vulkan_device.window_surface,
                                 g_vulkan_device.queue_family))
            {
...
void CreateDeviceAndQueue()
{
    ...
    VkResult succ = vkCreateDevice(
        g_vulkan_device.physical_device,
        &device_cinfo,
        nullptr,
...
void CreateSwapChain()
{
    ...
    for (int i = 0; i < ArraySize(surface_image_format); ++i)
    {
        ...
        {
            if (supported_format[j].format == surface_image_format[i] && supported_format[j].
                colorSpace == surface_color_space)
            {
    ...
    vkGetPhysicalDeviceSurfaceCapabilitiesKHR(g_vulkan_device.physical_device,
                                              g_vulkan_device.window_surface,
    ...
    VkResult succ = vkCreateSwapchainKHR(g_vulkan_device.device,
    ...
    for (size_t i = 0; i < g_vulkan_device.swapchain_image_count; i++)
    {
        ...
        succ = vkCreateImageView(g_vulkan_device.device,
                                 &info,
                                 nullptr,
                                 &g_vulkan_device.swapchain_image_views[i]);
    }
}
...
void DestroySwapchain()
{
    for (size_t i = 0; i < g_vulkan_device.swapchain_image_count; i++)
    {
        vkDestroyImageView(g_vulkan_device.device,
                           g_vulkan_device.swapchain_image_views[i],
                           nullptr);
        // vkDestroyFramebuffer(g_vulkan_device.device, g_vulkan_device.swapchain_freamebuffers[i], nullptr);
    }
...
void CreateDescriptorPool()
{
    ...
    VkResult succ = vkCreateDescriptorPool(g_vulkan_device.device,
                                           &pool_create_info,
                                           nullptr,
                                           &g_vulkan_device.descriptor_pool);
    check_vk(succ);
}
...
void InitGpuDevice(GpuCreateParam &param)
{
    ...
    VkQueryPoolCreateInfo pool_create_info = {VK_STRUCTURE_TYPE_QUERY_POOL_CREATE_INFO, nullptr, 0,
                                              VK_QUERY_TYPE_TIMESTAMP,
                                              param.gpu_time_queries_per_frame * 2u *
                                              MaxSwapchainImages,
                                              0};
    vkCreateQueryPool(g_vulkan_device.device,
                      &pool_create_info,
                      nullptr,
                      &g_vulkan_device.timestamp_query_pool);
