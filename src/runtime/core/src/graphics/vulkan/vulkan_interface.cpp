...
void CreateVkFramebuffers(const DeviceData &device_data, const RenderPipelineData& rp_data, WindowData &window_data)
{
	for (size_t i = 0; i < window_data.swapchain_image_count; i++)
	{
		...
		VkResult result =
			vkCreateFramebuffer(device_data.device, &framebuffer_info, nullptr, &window_data.swapchain_framebuffers[i]);
		check_vk(result);
	}
