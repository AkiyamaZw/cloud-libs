...
Selector::Selector(Registry *registry) : registry_ptr(registry) {}
...
#ifdef DEBUG
  std::cout << chunk->entity_live_flags->debug_format_to_string() << std::endl;
#endif
}

View Selector::create_view(Registry &registry, const MetaTypeList &metas,
                           MaskType mask) {
  View view{registry, mask};
  registry.for_each_matching_archetype(
      view.masks_, [&](internal::Archetype *arch) {
        std::vector<ComponentsView> res;
        for (auto chunk : arch->get_chunks()) {
